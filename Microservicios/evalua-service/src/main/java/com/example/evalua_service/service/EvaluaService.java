...
public class EvaluaService {
    ...
    public Map<String, Object> evaluateCredito(Long userId, Long creditId) {
        ...
        Credito ayuda = restTemplate.getForObject("http://localhost:8020/credito/" + creditId, Credito.class);
        if (ayuda == null) {
            throw new IllegalArgumentException("ERROR: SOLICITUD NO ENCONTRADA");
        }
        Credito solicitud = restTemplate.getForObject("http://localhost:8020/credito/" + creditId, Credito.class); // OBTENGO LA SOLICITUD DEL USUARIO -> VARA LA EVALUACIÓN -> ES EL MISMO QUE AYUDA, NO NECESITA VER SI EXISTE O  NO
        //-------------------------------------------------------------------------//
        ...
        if (Math.round(intmen) == Math.round(intanu / 12.0)) {
            ...
            Credito savedSolicitud = restTemplate.postForObject("http://localhost:8020/credito/save", request, Credito.class);
